  ...
  try {
    ...
    for (const slug of KNOWN_BLOG_SLUGS) {
      if (!dbSlugs.has(slug)) {
        ...
        allPages.push({ slug, title, wordCount: 0, content: '', publishedAt: '2026-07-01' });
        dbSlugs.add(slug);
      }
    }
...
function getComparisonDims(termA, termB, isES) {
  ...
  return [
    ...
    { label: isES ? 'Métrica Clave Afectada' : 'Key Metric Affected', valA: metaA.metricEN || '-', valB: metaB.metricEN || '-' },
    { label: isES ? 'Complejidad de Implementación' : 'Implementation Complexity', valA: metaA.effortEN || '-', valB: metaB.effortEN || '-' },
    { label: isES ? 'Tiempo para Ver Resultados' : 'Time to Impact', valA: metaA.timeEN || '-', valB: metaB.timeEN || '-' },
    { label: isES ? 'Mejor Etapa del Canal' : 'Best Channel Stage', valA: metaA.stageEN || '-', valB: metaB.stageEN || '-' },
...
function renderGlossaryComparison(slugA, slugB, isES) {
  ...
  const whenToUse = isES
    ? `<div class="card"><h2>🎯 ¿Cuándo usar cada uno?</h2><p><strong>Usa ${aName}:</strong> cuando necesites ${aDef.split('.')[0].toLowerCase()}. Es más efectivo para ${metaA.stageES || 'todas las etapas'} y tiene un impacto en ${metaA.metricES || 'múltiples métricas'}.</p><p><strong>Usa ${bName}:</strong> cuando tu objetivo sea ${bDef.split('.')[0].toLowerCase()}. Funciona mejor para ${metaB.stageES || 'todas las etapas'} y afecta principalmente a ${metaB.metricES || 'múltiples métricas'}.</p>${sameCat ? `<p style="margin-top:.8rem;color:#94a3b8;font-size:.9rem">💡 Ambos pertenecen a la misma categoría (${catName}), pero tienen enfoques complementarios. Úsalos juntos para maximizar resultados.</p>` : `<p style="margin-top:.8rem;color:#94a3b8;font-size:.9rem">💡 Pertenecen a categorías diferentes, lo que significa que cubren aspectos distintos del SEO en YouTube. Puedes trabajar en ambos simultáneamente.</p>`}</div>`
    : `<div class="card"><h2>🎯 When to Use Each</h2><p><strong>Use ${aName}:</strong> when you need to ${aDef.split('.')[0].toLowerCase()}. It is most effective for ${metaA.stageEN || 'all stages'} and primarily impacts ${metaA.metricEN || 'multiple metrics'}.</p><p><strong>Use ${bName}:</strong> when your goal is ${bDef.split('.')[0].toLowerCase()}. It works best for ${metaB.stageEN || 'all stages'} and primarily affects ${metaB.metricEN || 'multiple metrics'}.</p>${sameCat ? `<p style="margin-top:.8rem;color:#94a3b8;font-size:.9rem">💡 Both belong to the same category (${catName}) but have complementary approaches. Use them together for maximum results.</p>` : `<p style="margin-top:.8rem;color:#94a3b8;font-size:.9rem">💡 They belong to different categories, meaning they cover different aspects of YouTube SEO. You can work on both simultaneously.</p>`}</div>`;
