...
# HTML H2 splitter — matches <h2 ...>Heading</h2> (YT SEO Architect posts are full HTML)
HTML_H2_RE = re.compile(r"<h2[^>]*>(.*?)</h2>", re.DOTALL | re.IGNORECASE)
...
def split_sections(text: str) -> list[tuple[str, str]]:
    """Split into [(heading, body), ...] by H2. Handles markdown ## and HTML <h2>."""
    text = strip_frontmatter(text)
    # If content is HTML, split on <h2> tags instead of markdown ##
    if "<h2" in text.lower() and not text.strip().startswith("#"):
        html_parts = HTML_H2_RE.split(text)
        if len(html_parts) >= 2:
            intro = html_parts[0].strip()
            sections: list[tuple[str, str]] = []
            if intro:
                sections.append(("(intro)", intro))
            i = 1
            while i < len(html_parts) - 1:
                heading = re.sub(r"<[^>]+>", "", html_parts[i]).strip()
                body = html_parts[i + 1].strip()
