...
- HERO + OG images: `node scripts/generate-hero-scene.mjs <slug> "Title L1" "Title L2" "<keyword>" "BADGE" [--chart=<svg>] [--stats="L|V|D;..."]` — renders a dashboard scene via headless Chrome to `-hero.png` (800×400), `-hero.webp`, `-og.png` (1200×630). Each hero embeds the post's OWN first inline chart (or a hash-varied generated chart — type + 8 palettes via `scripts/gen-hero-chart.mjs`) with real stat cards from the post text (`scripts/extract-hero-data.py`). No text-on-card heroes, no identical heroes. Batch: `scripts/regenerate-all-heroes.sh`. PNGs gitignored → commit with `git add -f`.
