...
- HERO + OG images: `node scripts/generate-hero-scene.mjs <slug> "Title L1" "Title L2" "<keyword>" "BADGE"` — TOPIC-BASED scenes: classifies each post (slug+title) into 13 archetypes (SERP mockup, thumbnail card, Shorts reel, growth chart, retention curve, tag cluster, roadmap, audio/podcast, gaming, cooking, fitness, music, default) rendered via headless Chrome to `-hero.png` (800×400), `-hero.webp`, `-og.png` (1200×630). Per-post accent (8 colors) + hash-varied geometry; no identical heroes. Batch: `scripts/regenerate-all-heroes.sh`; overflow QA: `scripts/qa-hero-overflow.cjs` (needs NODE_PATH=~/qa/node_modules). PNGs gitignored → commit with `git add -f`.
