...
- HERO + OG images: generate with `node scripts/generate-hero-scene.mjs <slug> "Title L1" "Title L2" "<keyword>" "BADGE"` — renders a dashboard product-shot scene (retention curve + stat cards + keyword search chip) via headless Chrome to `-hero.png` (800×400), `-hero.webp`, and `-og.png` (1200×630). No text-on-card heroes. PNGs are gitignored → commit with `git add -f`.
